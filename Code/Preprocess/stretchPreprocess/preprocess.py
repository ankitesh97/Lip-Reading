#################################################
#This file is to create stretched data from original data
#stretched as in using the formula (i*orig_len/29)
#################################################
import os
import math
import cv2
    ...
    for setType in ['/test/','/train/','/val/']:
        ...
        for files in testSet:
            if files.endswith('.mp4'):
                ...
                out = cv2.VideoWriter(TARGET_DIRECTORY+word+setType+videoFileName.replace('mp4','avi'),fourcc,1,(72, 43),isColor=0)
                outColor = cv2.VideoWriter(COLOR_TARGET_DIRECTORY+word+setType+videoFileName.replace('mp4','avi'),fourcc,1,(72, 43),isColor=1)
                ctr = 0
                default_mouth_rects = []
                ...
                while(ctr<29):
                    ...
                    if ctr==28 and len(default_mouth_rects)==0:
                        ...
                        bakwasFile.write('./'+COLOR_TARGET_DIRECTORY+word+setType+videoFileName.replace('mp4','avi')+'\n')
                        bakwasFileFlag = True
                    ctr+=1
